...
if not whitelist[player.Name] then
    player:Kick("You are not whitelisted bro why u tryna crack xD dont ok? .")
end

local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/GeetankOnTop/lib2/refs/heads/main/lib2.txt", true))()

local window = library:AddWindow("KCE CLAN (Paid) ", {
    main_color = Color3.fromRGB(41, 74, 120), -- Color
    min_size = Vector2.new(525, 540), -- Size of the GUI lol
    can_resize = false,
})
...
local switch = Rebirth:AddSwitch("Fast Rebirths (Not For Free) ", function(state)
    if state then
    loadstring(game:HttpGet("https://gist.githubusercontent.com/GeetankOnTop/a0a8456f2901c19ea4e090857c5bd59f/raw/47860ddd0377e6d5a1c8d4b738224d70b571d20f/auto%2520reb"))()
    end
end)
...
local GUI = window:AddTab("GUI") 

GUI:AddButton("Enable Gui Kills", function()
    ...
    if leaderstats then
        ...
        if killsStat then
            task.spawn(function()
                while killGui.Parent do
                    killLabel.Text = "Kills: " .. tostring(killsStat.Value)
                    task.wait(0.1)
                end
            end)
        end
...
Credits:AddLabel("Made By HexaCore.py Published By Zeak")
Credits:AddLabel("For KCE Paid")